...
The following script automates the procedure described above.
It only calls the upgrade command when there is a mismatch between the latest and the current node image version.

[source,bash]
----
include::example$aks_maintenance.sh[]
----
